...
class Model: 
    ...
    def parameters(self):
        ...
        w8   = [3, 3, w1[3]+w2[3]+w5[3],                64     ]
        w9   = [1, 1, w8[3],                            w8[3]  ]
        w10  = [3, 3, w8[3],                            w8[3]  ]
        
        w11  = [3, 3, w1[3]+w2[3]+w5[3]+w8[3],          128     ]
        w12  = [1, 1, w11[3],                           w11[3] ]
        w13  = [3, 3, w11[3],                           w11[3] ]
        
        w14  = [3, 3, w1[3]+w2[3]+w5[3]+w8[3]+w11[3],   256    ]
        w15  = [1, 1, w14[3],                           w14[3] ]
        w16  = [3, 3, w14[3],                           w14[3] ]
        
        wFC  = [4*4*(w1[3]+w2[3]+w5[3]+w8[3]+w11[3]+w14[3]), 512]    
        wOut = [wFC[1], 1108]
        ...
        params_w = {
                    'w1'   : tf.Variable(tf.truncated_normal( w1    , stddev = 0.1 )),  
                    'w2'   : tf.Variable(tf.truncated_normal( w2    , stddev = 0.1 )),  
                    'w3'   : tf.Variable(tf.truncated_normal( w3    , stddev = 0.1 )),
                    'w4'   : tf.Variable(tf.truncated_normal( w4    , stddev = 0.1 )),  
                    'w5'   : tf.Variable(tf.truncated_normal( w5    , stddev = 0.1 )),  
                    'w6'   : tf.Variable(tf.truncated_normal( w6    , stddev = 0.1 )),  
                    'w7'   : tf.Variable(tf.truncated_normal( w7    , stddev = 0.1 )),   
                    'w8'   : tf.Variable(tf.truncated_normal( w8    , stddev = 0.1 )),  
                    'w9'   : tf.Variable(tf.truncated_normal( w9    , stddev = 0.1 )),  
                    'w10'  : tf.Variable(tf.truncated_normal( w10   , stddev = 0.1 )),  
                    'w11'  : tf.Variable(tf.truncated_normal( w11   , stddev = 0.1 )),   
                    'w12'  : tf.Variable(tf.truncated_normal( w12   , stddev = 0.1 )),  
                    'w13'  : tf.Variable(tf.truncated_normal( w13   , stddev = 0.1 )),  
                    'w14'  : tf.Variable(tf.truncated_normal( w14   , stddev = 0.1 )),   
                    'w15'  : tf.Variable(tf.truncated_normal( w15   , stddev = 0.1 )),  
                    'w16'  : tf.Variable(tf.truncated_normal( w16   , stddev = 0.1 )),  
                    'wFC'  : tf.Variable(tf.truncated_normal( wFC   , stddev = 0.1 )),  
                    'wOut' : tf.Variable(tf.truncated_normal( wOut  , stddev = 0.1 ))                    
                   }
        params_b = {
                    'b1'   : tf.Variable(tf.truncated_normal( [b1  ], stddev = 0.1 )),  
                    'b2'   : tf.Variable(tf.truncated_normal( [b2  ], stddev = 0.1 )),  
                    'b3'   : tf.Variable(tf.truncated_normal( [b3  ], stddev = 0.1 )), 
                    'b4'   : tf.Variable(tf.truncated_normal( [b4  ], stddev = 0.1 )), 
                    'b5'   : tf.Variable(tf.truncated_normal( [b5  ], stddev = 0.1 )), 
                    'b6'   : tf.Variable(tf.truncated_normal( [b6  ], stddev = 0.1 )), 
                    'b7'   : tf.Variable(tf.truncated_normal( [b7  ], stddev = 0.1 )), 
                    'b8'   : tf.Variable(tf.truncated_normal( [b8  ], stddev = 0.1 )), 
                    'b9'   : tf.Variable(tf.truncated_normal( [b9  ], stddev = 0.1 )), 
                    'b10'  : tf.Variable(tf.truncated_normal( [b10 ], stddev = 0.1 )), 
                    'b11'  : tf.Variable(tf.truncated_normal( [b11 ], stddev = 0.1 )), 
                    'b12'  : tf.Variable(tf.truncated_normal( [b12 ], stddev = 0.1 )), 
                    'b13'  : tf.Variable(tf.truncated_normal( [b13 ], stddev = 0.1 )), 
                    'b14'  : tf.Variable(tf.truncated_normal( [b14 ], stddev = 0.1 )), 
                    'b15'  : tf.Variable(tf.truncated_normal( [b15 ], stddev = 0.1 )), 
                    'b16'  : tf.Variable(tf.truncated_normal( [b16 ], stddev = 0.1 )),  
                    'bFC'  : tf.Variable(tf.truncated_normal( [bFC ], stddev = 0.1 )),     
                    'bOut' : tf.Variable(tf.truncated_normal( [bOut], stddev = 0.1 ))
                   }
    ...
    def loss_function(self):
        ...
        for w in self.params_w:
            weights_penalty += tf.nn.l2_loss(self.params_w[w]) * 0.0001
